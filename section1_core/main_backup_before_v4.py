...
def _load_draws(db: Session = None) -> pd.DataFrame:
    ...
    2) Si no existe y hay DB, hace fallback a la tabla DrawResult.
    """
    ...
    if os.path.exists(csv_path):
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Error leyendo draws CSV: {e}")
...
def _insights(draws_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    whites = []
    for c in WHITE_COLS:
        whites.extend(draws_df[c].tolist())

    white_freq = Counter(whites)
    pb_freq = Counter(draws_df["pb"].tolist())

    tmp = draws_df.copy()
    tmp["sum_white"] = tmp[WHITE_COLS].sum(axis=1)
    avg_sum = float(tmp["sum_white"].mean())

    # devuelvo listas ordenadas por frecuencia
    white_list = [{"number": k, "frequency": v} for k, v in white_freq.most_common()]
    pb_list = [{"pb": k, "frequency": v} for k, v in pb_freq.most_common()]

    return {
        "count": int(len(draws_df)),
...
def export_first_position_endpoint():
    try:
        export_by_first_position()
        return {"ok": True, "message": "Export first position ejecutado."}
    except Exception as e:
        raise HTTPException(500, f"Error export_first_position: {e}")

# ============================================================
#   COMPARE (API + UI) — ÚNICA VERSIÓN
# ============================================================
