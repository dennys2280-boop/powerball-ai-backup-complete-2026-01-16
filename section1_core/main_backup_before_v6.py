...
# ============================================================
#   UPGRADE 1: EXPORTS (CSV / EXCEL) for by-date endpoints
#   - draws/by-date
#   - insights/by-date
#   - compare/by-date
#   - compare/by-date/multi
#   Uses StreamingResponse so /docs downloads work.
# ============================================================

def _df_to_csv_stream(df: pd.DataFrame, filename: str) -> StreamingResponse:
    # Pandas escribe texto; BytesIO espera bytes. Usamos StringIO y luego codificamos a UTF-8.
    sio = StringIO()
...
def _draws_day_df(month: int, day: int, db: Session = None) -> pd.DataFrame:
    df = _load_draws(db)
...
def _compare_day_df(month: int, day: int, ticket: str, db: Session = None) -> pd.DataFrame:
    ...
    if results.empty:
        return results
    results["draw_date"] = pd.to_datetime(results["draw_date"]).dt.strftime("%Y-%m-%d")
    return results

def _compare_multi_day_df(month: int, day: int, db: Session = None) -> pd.DataFrame:
    df_draws = _load_draws(db)
...
# ---------------------------
# ---------------------------
# Helpers: robust paths for CSV files
# ---------------------------
...
def _normalize_draws_df(df: pd.DataFrame) -> pd.DataFrame:
    """Normaliza columnas de sorteos a: draw_date, n1..n5, pb.
    Acepta variaciones comunes de nombres de columnas en CSV."""
    df = df.copy()
    # normaliza nombres
    df.columns = [str(c).strip() for c in df.columns]
    cols_l = {c.lower().strip().replace(" ", "_"): c for c in df.columns}
    ...
    # draw_date
    draw_col = _pick("draw_date", "drawdate", "date", "draw_date_", "draw-date", "draw date", "fecha")
    if draw_col is None:
        raise HTTPException(status_code=422, detail=f"CSV draws no tiene columna de fecha (esperaba draw_date/date). Columnas: {list(df.columns)}")
    if draw_col != "draw_date":
        df = df.rename(columns={draw_col: "draw_date"})

    # white balls
    mapping = {}
    for i, aliases in enumerate([
        ("n1","wn1","white1","w1","ball1","b1","num1"),
        ("n2","wn2","white2","w2","ball2","b2","num2"),
        ("n3","wn3","white3","w3","ball3","b3","num3"),
        ("n4","wn4","white4","w4","ball4","b4","num4"),
        ("n5","wn5","white5","w5","ball5","b5","num5"),
    ], start=1):
        col = None
        for a in aliases:
            col = _pick(a)
            if col:
                break
        if col is None:
            # si faltan, salimos con error claro
            raise HTTPException(status_code=422, detail=f"CSV draws falta columna para n{i} (alias probados: {list(aliases)}). Columnas: {list(df.columns)}")
        if col != f"n{i}":
            mapping[col] = f"n{i}"

    pb_col = _pick("pb", "powerball", "winning_powerball", "power_ball", "pball", "p")
    if pb_col is None:
        raise HTTPException(status_code=422, detail=f"CSV draws falta columna pb/powerball. Columnas: {list(df.columns)}")
    if pb_col != "pb":
        mapping[pb_col] = "pb"
    ...
    return df[["draw_date","n1","n2","n3","n4","n5","pb"]]


def _coerce_draw_date(series: pd.Series) -> pd.Series:
    """Convierte una columna a datetime, probando formatos comunes."""
    s = series.copy()
    # ya datetime?
    if hasattr(s, "dt"):
        try:
            # si es datetime64 ya, devuelve
            _ = s.dt.year
            return s
        except Exception:
            pass

    # primer intento: infer
    ...
    for fmt in ("%Y-%m-%d", "%m/%d/%Y", "%m-%d-%Y", "%Y/%m/%d", "%d/%m/%Y", "%d-%m-%Y"):
        dt2 = pd.to_datetime(s, errors="coerce", format=fmt)
        if dt2.notna().any():
            return dt2
...
def _load_draws(db: Session = None) -> pd.DataFrame:
    """
    1) Intenta leer powerball_draws.csv usando ruta absoluta (robusto ante cwd).
    2) Si no existe y hay DB, hace fallback a tabla DrawResult.
    Devuelve columnas normalizadas: draw_date(datetime), n1..n5(int), pb(int) y agrega year/day/month en _filter_draws_by_day.
    """
    csv_path = _abs_path(POWERBALL_DRAWS_CSV)

    if os.path.exists(csv_path):
        try:
            raw = pd.read_csv(csv_path)
            df = _normalize_draws_df(raw)

            df["draw_date"] = _coerce_draw_date(df["draw_date"])
            if df["draw_date"].isna().all():
                sample = raw.iloc[:5].to_dict(orient="records")
                raise HTTPException(
                    status_code=422,
                    detail=f"draw_date no tiene valores de fecha válidos. Ejemplo primeras filas: {sample}",
                )

            df["draw_date"] = pd.to_datetime(df["draw_date"])
            # ints (si hay NaN, los dejamos como NaN y luego filtramos)
            for c in ["n1","n2","n3","n4","n5","pb"]:
                df[c] = pd.to_numeric(df[c], errors="coerce")
            # elimina filas claramente inválidas
            df = df.dropna(subset=["draw_date","n1","n2","n3","n4","n5","pb"])
            return df
...
def _load_tickets(db: Session = None) -> pd.DataFrame:
    """
    Carga los tickets.
    1) Intenta leer tickets.csv con ruta absoluta.
    2) Si no existe y hay DB, hace fallback a la tabla Ticket.
    """
    csv_path = _abs_path(TICKETS_CSV)

    if os.path.exists(csv_path):
        try:
            df = pd.read_csv(csv_path)
        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Error leyendo tickets CSV: {e}")
...
def draws_by_date_export_csv(
    ...
    out2 = _draws_day_df(month, day, db=db)
    filename = f"draws_by_date_{month:02d}-{day:02d}.csv"
    return _df_to_csv_stream(out2, filename)
...
def draws_by_date_export_xlsx(
    ...
    return _df_to_xlsx_stream({"draws": out2}, filename)
...
def insights_by_date_export_csv(
    ...
    # CSV en formato "long" (category, number, frequency) + summary
    rows = []
    rows.append({
        "category": "summary",
        "metric": "count",
...
def insights_by_date_export_xlsx(
    ...
    df_white = pd.DataFrame(ins.get("white_frequency") or [])
    if not df_white.empty:
        df_white = df_white.rename(columns={"number": "number", "frequency": "frequency"})

    df_pb = pd.DataFrame(ins.get("pb_frequency") or [])
    if not df_pb.empty:
        df_pb = df_pb.rename(columns={"pb": "pb", "frequency": "frequency"})
